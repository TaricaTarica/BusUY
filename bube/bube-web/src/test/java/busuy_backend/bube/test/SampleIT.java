package busuy_backend.bube.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "bube-ear.ear");

        // Current directory is the root of the "bube-web" project. Go up one level
        // and enter the "bube-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../bube-ejb/target/bube-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "bube-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/bube-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "bube-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
        ...
        war.addPackage("busuy_backend.bube.test");
